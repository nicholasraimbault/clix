...
async fn serve_conn(
    ...
    let response = match dispatch(&store, &peer, req).await {
        Ok(v) => v,
...
async fn dispatch(store: &Arc<Mutex<Store>>, peer: &Peer, req: Value) -> Result<Value> {
    match req.get("op").and_then(Value::as_str).unwrap_or("") {
        "exec" => {
            ...
            exec::submit(store, &peer.name, &argv, id)
        }
        "job_get" => {
            let id = req
                .get("job_id")
        ...
        "pin_list" => crate::pin::rpc_list(&store.lock().unwrap_or_else(|e| e.into_inner()), peer),
        "pin_get" => crate::pin::rpc_get(&store.lock().unwrap_or_else(|e| e.into_inner()), &req),
        "pin_put" => crate::pin::rpc_put(&store.lock().unwrap_or_else(|e| e.into_inner()), &req),
        "pin_commit" => crate::pin::rpc_commit(
